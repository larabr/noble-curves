...
const fe = (x) => x instanceof BigInteger && _0n.lt(x) && x.lt(secp256k1P);
const ge = (x) => x instanceof BigInteger && _0n.lt(x) && x.lt(secp256k1N);
...
function lift_x(x) {
    ...
    if (!y.isEven())
        y = modP(y.negate()); // Return the unique point P such that x(P) = x and
...
function schnorrVerify(signature, message, publicKey) {
    ...
    try {
        ...
        if (!R || !R.hasEvenY() || !R.toAffine().x.equal(r))
...
].map((i) => i.map((j) => BigInteger.new(j)))))();
