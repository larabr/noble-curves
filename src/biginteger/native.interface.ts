...
export default class NativeBigInteger extends AbstractBigInteger {
   ...
   */
  modExp(e: NativeBigInteger, n: NativeBigInteger) {
    if (n.isZero()) throw Error('Modulo cannot be zero');
    if (n.isOne()) return new NativeBigInteger(0);
   ...
   */
  private _egcd(bInput: NativeBigInteger) {
    let x = BigInt(0);
    let y = BigInt(1);
    let xPrev = BigInt(1);
    let yPrev = BigInt(0);
   ...
   * @param {BigInteger} x - Shift value
   */
  irightShift(x: NativeBigInteger) {
    this.value >>= x.value;
    return this;
  }
